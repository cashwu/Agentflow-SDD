...
The repository SHALL provide `install-cash-skills.fish --target <project> [--dry-run] [--force]`. The installer MUST validate the complete 24-file source inventory, the target directory, and every destination conflict before its first target write.

#### Scenario: Install into a clean target

- **WHEN** the installer receives a valid target with no cash destinations
- **THEN** it installs all 24 canonical skill files
- **AND** every installed file is byte-identical to its source
- **AND** it exits with code 0

#### Scenario: Identical target is unchanged

- **WHEN** all managed target files are byte-identical to the source
- **THEN** the installer reports all destinations as unchanged
- **AND** it exits with code 0

#### Scenario: Conflict fails before writes

- **GIVEN** at least one managed destination differs from its source
- **WHEN** the installer runs without `--force`
- **THEN** it exits non-zero and identifies every conflicting destination
- **AND** it does not install or replace any managed destination

#### Scenario: Force replaces only managed destinations

- **WHEN** the installer runs with `--force`
- **THEN** it replaces differing managed cash destinations
- **AND** it preserves every file outside the explicit 24-file inventory

#### Scenario: Dry run has no persistent effects

- **WHEN** the installer runs with `--dry-run`
- **THEN** it reports install, unchanged, replace, and conflict actions using normal preflight rules
- **AND** it does not create a target directory, target temporary file, registry, cache, lock, LaunchAgent, or background process
...
The cash installer MUST NOT register targets, compute freshness fingerprints, schedule repair, install a LaunchAgent, or modify a Spectra-managed skill. Cash skill maintenance SHALL occur only through explicit source changes and explicit installer invocations.

#### Scenario: Completed cash installation

- **WHEN** a cash installation succeeds
- **THEN** no persistent state exists outside the target cash skill files
- **AND** no future process is scheduled
- **AND** a later source change does not propagate until the installer is invoked again
...
`scripts/cash-skills/tests/skill-checks.fish` SHALL verify inventory, metadata, namespace routing, quality-gate markers, installer branches, cleanup branches, legacy removal, variant parity, and forced Spectra update isolation. The suite MUST use isolated targets, an isolated `HOME`, and a stubbed `launchctl` for mutating test cases.

#### Scenario: Full regression suite passes

- **WHEN** the cash contract suite runs against a compliant repository
- **THEN** every required branch passes
- **AND** no actual user LaunchAgent, registry, cache, or external project is modified

#### Scenario: Contract drift fails loudly

- **WHEN** a managed cash file, invocation, installer branch, cleanup branch, or isolation invariant violates this specification
- **THEN** the suite exits non-zero
- **AND** its diagnostics identify the failed invariant and relevant project-relative path
...
The installer SHALL canonicalize an existing target and MUST reject an empty target, an unresolved target, or `/`. Before any target write it MUST reject a managed destination or existing managed parent that is a symlink and MUST prove that every resolved destination remains below the canonical target. The cleanup SHALL validate that `HOME` is non-empty, absolute, and not `/`; SHALL resolve only the exact known cleanup paths below that HOME; and MUST reject a symlinked cleanup target or existing parent before invoking `launchctl` or removing state. Every boundary failure MUST fail closed with zero writes.

#### Scenario: Installer rejects a symlink escape before writes

- **GIVEN** a managed target parent or destination is a symlink
- **WHEN** `install-cash-skills.fish` performs preflight
- **THEN** it exits non-zero before creating or replacing any target file
- **AND** identifies the unsafe project-relative destination
...
- **AND** does not invoke `launchctl` or remove any file
...
The repository SHALL replace `SPECTRA-PLUS.md` with `CASH-SKILLS.md`. The cash document MUST list the dual-variant inventory, explain explicit cross-project installation, document cleanup dry-run and the order “install every registered target, then remove the schedule,” and state that cash skills have no periodic repair. `openspec/signals/README.md` MUST describe current writers as cash review loops while preserving historical `## Occurrences` provenance text.
...
- **AND** no current instruction tells the user to generate or periodically repair plus skills
