...
- 2026-08-22 — default-spec-sync-on-archive — cash-apply round 2 — round 1 在「負向判準（實作前 exit 0，實作後 MUST exit 1）」區塊新增了一條釘住自己中間態措辭的判準，該字串從未存在於 HEAD baseline，因此在 baseline 即 exit 1、從無 red 狀態，對 baseline 零鑑別力；修正為移出負向區塊、另立明示極性的「回歸守則」。
- 2026-08-22 — guard-task-state-integrity — cash-propose round 2 — round 1 在「負向判準（實作前 exit 0，實作後 MUST exit 1）」區塊新增一條釘住自己中間態措辭的判準，該字串從未存在於 HEAD baseline，因此在 baseline 即 exit 1、從無 red 狀態；round 4 另有一條計數判準被放進負向區塊，但依 IC 實作後會 exit 0，宣告方向與 IC 直接矛盾。
