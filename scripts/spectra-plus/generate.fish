#!/usr/bin/env -S fish --no-config

set script_path (status --current-filename)
set script_dir (realpath (dirname "$script_path"))
set script_path "$script_dir/generate.fish"
set root_dir (realpath "$script_dir/../..")
set rules_file "$script_dir/rules.yaml"
set template_dir "$script_dir/template"
set marker "<!-- Generated by scripts/spectra-plus/generate.fish — DO NOT EDIT MANUALLY -->"

function usage
    echo "Usage: generate.fish [--root <dir>] [--fingerprints | skill-name]"
    echo
    echo "Regenerates plus skill SKILL.md files from spectra skills, rules.yaml, and templates."
    echo "Pass --root to target a different project directory (defaults to the repo containing this script)."
    echo "Pass one skill name, such as spectra-propose-plus, to regenerate only that output."
    echo "All variants (claude, codex, ...) defined under the skill are produced together."
    echo "Pass --fingerprints for a read-only four-row TSV query of expected fingerprints."
end

function die
    set code $argv[1]
    set message $argv[2..-1]
    echo $message >&2
    exit $code
end

function require_yq
    if not command -q yq
        die 2 "rules.yaml parse error: yq is required"
    end
end

function yq_value
    set expression $argv[1]
    yq -r "$expression" "$rules_file"
end

function plus_skill_names
    # These generated plus skills intentionally share one freshness marker.
    echo spectra-propose-plus
    echo spectra-apply-plus
end

function fingerprint_skill_names
    # Bytewise ascending output order for the query protocol.
    echo spectra-apply-plus
    echo spectra-propose-plus
end

function valid_iso_date --argument-names value
    string match -qr '^[0-9]{4}-[0-9]{2}-[0-9]{2}$' -- "$value"; or return 1

    set parsed (date -j -f "%Y-%m-%d" "$value" "+%Y-%m-%d" 2>/dev/null)
    if test $status -eq 0; and test "$parsed" = "$value"
        return 0
    end

    set parsed (date -d "$value" "+%Y-%m-%d" 2>/dev/null)
    if test $status -eq 0; and test "$parsed" = "$value"
        return 0
    end

    return 1
end

function validate_rules
    require_yq

    if not test -f "$rules_file"
        die 2 "rules.yaml parse error: missing $rules_file"
    end

    set err (yq '.' "$rules_file" 2>&1 >/dev/null)
    if test $status -ne 0
        die 2 "rules.yaml parse error: $err"
    end

    if not yq -e '.skills | type == "!!map"' "$rules_file" >/dev/null
        die 2 "rules.yaml parse error: missing field skills"
    end
end

function validate_plus_metadata_consistency
    set plus_skills (plus_skill_names)

    for field in spectraPlusVersion spectraPlusUpdated
        for skill in $plus_skills
            if not yq -e ".skills.\"$skill\".metadata | has(\"$field\")" "$rules_file" >/dev/null
                die 2 "rules.yaml parse error: $skill metadata missing field $field"
            end
            validate_plus_metadata_value "$skill" "$field"
        end

        set reference_skill $plus_skills[1]
        set reference_value (yq_value ".skills.\"$reference_skill\".metadata.\"$field\"")
        for skill in $plus_skills[2..-1]
            set skill_value (yq_value ".skills.\"$skill\".metadata.\"$field\"")
            if test "$reference_value" != "$skill_value"
                die 2 "rules.yaml parse error: mismatched plus metadata field $field"
            end
        end
    end
end

function validate_plus_metadata_value
    set skill $argv[1]
    set field $argv[2]

    if not yq -e ".skills.\"$skill\".metadata.\"$field\" | type == \"!!str\"" "$rules_file" >/dev/null
        die 2 "rules.yaml parse error: $skill metadata field $field must be a non-empty string"
    end

    set value (yq_value ".skills.\"$skill\".metadata.\"$field\"")
    if test -z (string trim -- "$value")
        die 2 "rules.yaml parse error: $skill metadata field $field must be a non-empty string"
    end
end

function validate_skill
    set skill $argv[1]

    for field in metadata transformations variants
        if not yq -e ".skills.\"$skill\" | has(\"$field\")" "$rules_file" >/dev/null
            die 2 "rules.yaml parse error: $skill missing field $field"
        end
    end

    for field in name description spectraPlusVersion spectraPlusUpdated
        if not yq -e ".skills.\"$skill\".metadata | has(\"$field\")" "$rules_file" >/dev/null
            die 2 "rules.yaml parse error: $skill metadata missing field $field"
        end
    end

    validate_plus_metadata_value "$skill" spectraPlusVersion
    validate_plus_metadata_value "$skill" spectraPlusUpdated

    set updated (yq_value ".skills.\"$skill\".metadata.spectraPlusUpdated")
    if not valid_iso_date "$updated"
        die 2 "rules.yaml parse error: $skill metadata field spectraPlusUpdated must be a valid YYYY-MM-DD date"
    end

    if not yq -e ".skills.\"$skill\".variants | type == \"!!map\"" "$rules_file" >/dev/null
        die 2 "rules.yaml parse error: $skill variants must be a map"
    end

    set variant_count (yq -r ".skills.\"$skill\".variants | length" "$rules_file")
    if test "$variant_count" = "0"
        die 2 "rules.yaml parse error: $skill variants is empty"
    end

    for variant in (yq -r ".skills.\"$skill\".variants | keys | .[]" "$rules_file")
        for field in source output
            if not yq -e ".skills.\"$skill\".variants.\"$variant\" | has(\"$field\")" "$rules_file" >/dev/null
                die 2 "rules.yaml parse error: $skill variant $variant missing field $field"
            end
        end

        if yq -e ".skills.\"$skill\".variants.\"$variant\" | has(\"substitutions\")" "$rules_file" >/dev/null 2>/dev/null
            set sub_count (yq -r ".skills.\"$skill\".variants.\"$variant\".substitutions | length" "$rules_file")
            if test "$sub_count" != "0"
                for sidx in (seq 0 (math $sub_count - 1))
                    for sfield in from to
                        if not yq -e ".skills.\"$skill\".variants.\"$variant\".substitutions[$sidx] | has(\"$sfield\")" "$rules_file" >/dev/null
                            die 2 "rules.yaml parse error: $skill variant $variant substitutions[$sidx] missing field $sfield"
                        end
                    end
                end
            end
        end
    end

    set count (yq_value ".skills.\"$skill\".transformations | length")
    if test "$count" = "0"
        die 2 "rules.yaml parse error: $skill transformations is empty"
    end

    for index in (seq 0 (math $count - 1))
        for field in target_section operation
            if not yq -e ".skills.\"$skill\".transformations[$index] | has(\"$field\")" "$rules_file" >/dev/null
                die 2 "rules.yaml parse error: $skill transformations[$index] missing field $field"
            end
        end

        set op (yq_value ".skills.\"$skill\".transformations[$index].operation")
        if not contains "$op" replace append remove
            die 2 "rules.yaml parse error: $skill transformations[$index] operation"
        end

        if contains "$op" replace append
            if not yq -e ".skills.\"$skill\".transformations[$index] | has(\"template\")" "$rules_file" >/dev/null
                die 2 "rules.yaml parse error: $skill transformations[$index] missing field template"
            end

            set template_name (yq_value ".skills.\"$skill\".transformations[$index].template")
            if not test -f "$template_dir/$template_name"
                die 3 "missing template file: $template_name"
            end
        end
    end
end

function validate_all_skills
    for skill in (plus_skill_names)
        validate_skill "$skill"
    end
end

function referenced_template_names
    set referenced
    for skill in (plus_skill_names)
        set count (yq_value ".skills.\"$skill\".transformations | length")
        for index in (seq 0 (math $count - 1))
            set op (yq_value ".skills.\"$skill\".transformations[$index].operation")
            if contains "$op" replace append
                set template_name (yq_value ".skills.\"$skill\".transformations[$index].template")
                if not contains -- "$template_name" $referenced
                    set referenced $referenced "$template_name"
                end
            end
        end
    end
    printf '%s\n' $referenced
end

function checksum_fields --argument-names input_path label
    if not test -f "$input_path"
        echo "fingerprint input not found: $label" >&2
        return 2
    end

    set checksum_output (command cksum < "$input_path")
    set checksum_status $status
    if test "$checksum_status" -ne 0
        echo "fingerprint cksum failed for $label" >&2
        return 2
    end

    set fields (string split -n ' ' -- (string trim -- "$checksum_output"))
    if test (count $fields) -ne 2; or not string match -qr '^[0-9]+$' -- "$fields[1]"; or not string match -qr '^[0-9]+$' -- "$fields[2]"
        echo "fingerprint cksum returned malformed output for $label" >&2
        return 2
    end

    printf '%s\t%s\n' "$fields[1]" "$fields[2]"
end

function compute_fingerprint --argument-names skill variant
    if not command -q cksum
        echo "fingerprint error: cksum is required" >&2
        return 2
    end

    set source_rel (yq_value ".skills.\"$skill\".variants.\"$variant\".source")
    set labels \
        "base:$source_rel" \
        "generator:scripts/spectra-plus/generate.fish" \
        "rules:scripts/spectra-plus/rules.yaml"
    set paths \
        "$root_dir/$source_rel" \
        "$script_path" \
        "$rules_file"

    for template_name in (referenced_template_names)
        set labels $labels "template:scripts/spectra-plus/template/$template_name"
        set paths $paths "$template_dir/$template_name"
    end

    set composite_lines
    for index in (seq 1 (count $paths))
        set fields (checksum_fields "$paths[$index]" "$labels[$index]")
        or return $status
        set checksum_parts (string split (printf '\t') -- "$fields")
        if test (count $checksum_parts) -ne 2
            echo "fingerprint internal error for $labels[$index]" >&2
            return 2
        end
        set composite_lines $composite_lines (printf '%s\t%s\t%s' "$labels[$index]" "$checksum_parts[1]" "$checksum_parts[2]")
    end

    set composite_checksum (printf '%s\n' $composite_lines | command cksum)
    set checksum_status $status
    if test "$checksum_status" -ne 0
        echo "fingerprint cksum failed for composite $skill/$variant" >&2
        return 2
    end

    set final_fields (string split -n ' ' -- (string trim -- "$composite_checksum"))
    if test (count $final_fields) -ne 2; or not string match -qr '^[0-9]+$' -- "$final_fields[1]"
        echo "fingerprint cksum returned malformed output for composite $skill/$variant" >&2
        return 2
    end
    echo "$final_fields[1]"
end

function find_section_end
    set file $argv[1]
    set start $argv[2]

    set end (awk -v start="$start" 'NR > start && ($0 ~ /^[0-9]+\. \*\*/ || $0 ~ /^\*\*/) { print NR - 1; exit } END { if (!found) { } }' "$file")
    if test -z "$end"
        set end (wc -l < "$file" | string trim)
    end
    echo "$end"
end

function apply_transform
    set skill $argv[1]
    set source_path $argv[2]
    set input_path $argv[3]
    set output_path $argv[4]
    set target $argv[5]
    set op $argv[6]
    set template_name $argv[7]

    set start_line (rg -n --fixed-strings "$target" "$input_path" | head -n 1 | cut -d: -f1)
    if test -z "$start_line"
        echo "target_section not found for $skill: $target in $source_path" >&2
        return 1
    end

    set end_line (find_section_end "$input_path" "$start_line")
    set template_path ""
    if contains "$op" replace append
        set template_path "$template_dir/$template_name"
    end

    awk \
        -v start="$start_line" \
        -v end="$end_line" \
        -v op="$op" \
        -v template_path="$template_path" '
        BEGIN {
            if (template_path != "") {
                while ((getline line < template_path) > 0) {
                    template = template line "\n"
                }
                close(template_path)
            }
        }
        NR < start || NR > end {
            print
            next
        }
        NR == start {
            if (op == "replace") {
                printf "%s\n", template
            } else if (op == "append") {
                print
            }
            next
        }
        NR > start && NR <= end {
            if (op == "append") {
                print
                if (NR == end) {
                    printf "\n%s\n", template
                }
            }
            next
        }
        ' "$input_path" > "$output_path"
end

function renumber_step_headings
    set input_path $argv[1]
    set output_path $argv[2]

    awk '
        /^[0-9]+\. \*\*/ {
            sub(/^[0-9]+\./, ++step ".")
        }
        { print }
    ' "$input_path" > "$output_path"
end

function set_frontmatter_field
    set input_path $argv[1]
    set output_path $argv[2]
    set key $argv[3]
    set value $argv[4]

    awk -v key="$key" -v value="$value" '
        BEGIN { in_fm = 0; done = 0 }
        NR == 1 && $0 == "---" { in_fm = 1; print; next }
        in_fm && $0 == "---" {
            if (!done) {
                print key ": " value
            }
            in_fm = 0
            print
            next
        }
        in_fm && $0 ~ "^" key ":" {
            print key ": " value
            done = 1
            next
        }
        { print }
    ' "$input_path" > "$output_path"
end

function remove_frontmatter_field
    set input_path $argv[1]
    set output_path $argv[2]
    set key $argv[3]

    awk -v key="$key" '
        BEGIN { in_fm = 0 }
        NR == 1 && $0 == "---" { in_fm = 1; print; next }
        in_fm && $0 == "---" { in_fm = 0; print; next }
        in_fm && $0 ~ "^" key ":" { next }
        { print }
    ' "$input_path" > "$output_path"
end

function set_frontmatter_metadata_field
    set input_path $argv[1]
    set output_path $argv[2]
    set key $argv[3]
    set value $argv[4]

    awk -v key="$key" -v value="$value" '
        BEGIN { in_fm = 0; in_metadata = 0; saw_metadata = 0; done = 0 }
        NR == 1 && $0 == "---" { in_fm = 1; print; next }
        in_fm && $0 == "metadata:" {
            saw_metadata = 1
            in_metadata = 1
            print
            next
        }
        in_metadata && $0 ~ "^  " key ":" {
            print "  " key ": " value
            done = 1
            next
        }
        in_metadata && ($0 == "---" || $0 ~ "^[^ ]") {
            if (!done) {
                print "  " key ": " value
                done = 1
            }
            in_metadata = 0
        }
        in_fm && $0 == "---" {
            if (!saw_metadata) {
                print "metadata:"
                print "  " key ": " value
                done = 1
            }
            in_fm = 0
            print
            next
        }
        { print }
    ' "$input_path" > "$output_path"
end

function apply_literal_substitution
    set input_path $argv[1]
    set output_path $argv[2]
    set from $argv[3]
    set to $argv[4]

    awk -v from="$from" -v to="$to" '
        {
            line = $0
            out = ""
            flen = length(from)
            while ((idx = index(line, from)) > 0) {
                out = out substr(line, 1, idx - 1) to
                line = substr(line, idx + flen)
            }
            out = out line
            print out
        }
    ' "$input_path" > "$output_path"
end

function apply_substitutions
    set skill $argv[1]
    set variant $argv[2]
    set input_path $argv[3]
    set output_path $argv[4]

    if not yq -e ".skills.\"$skill\".variants.\"$variant\" | has(\"substitutions\")" "$rules_file" >/dev/null 2>/dev/null
        command cp -f "$input_path" "$output_path"
        return 0
    end

    set sub_count (yq -r ".skills.\"$skill\".variants.\"$variant\".substitutions | length" "$rules_file")
    if test "$sub_count" = "0"
        command cp -f "$input_path" "$output_path"
        return 0
    end

    set work_path (mktemp "/tmp/spectra-plus-sub-$skill-$variant.XXXXXX")
    command cp -f "$input_path" "$work_path"

    for index in (seq 0 (math $sub_count - 1))
        set from (yq_value ".skills.\"$skill\".variants.\"$variant\".substitutions[$index].from")
        set to (yq_value ".skills.\"$skill\".variants.\"$variant\".substitutions[$index].to")
        set next_path (mktemp "/tmp/spectra-plus-sub-$skill-$variant.XXXXXX")
        apply_literal_substitution "$work_path" "$next_path" "$from" "$to"
        command mv -f "$next_path" "$work_path"
    end

    command mv -f "$work_path" "$output_path"
end

function insert_marker_after_frontmatter
    set input_path $argv[1]
    set output_path $argv[2]

    awk -v marker="$marker" '
        BEGIN { fm_state = 0 }
        NR == 1 && $0 == "---" { print; fm_state = 1; next }
        fm_state == 1 && $0 == "---" {
            print
            print marker
            fm_state = 2
            next
        }
        { print }
        END {
            if (fm_state != 2) {
                print "insert_marker_after_frontmatter: no closing --- found" > "/dev/stderr"
                exit 1
            }
        }
    ' "$input_path" > "$output_path"
end

function validate_generated
    set output_path $argv[1]
    set skill $argv[2]

    for field in name description
        if not frontmatter_has_key "$output_path" "$field"
            die 2 "generated frontmatter missing field $field for $skill"
        end
    end

    for field in spectraPlusVersion spectraPlusUpdated spectraPlusFingerprint
        if not frontmatter_has_metadata_key "$output_path" "$field"
            die 2 "generated frontmatter metadata missing field $field for $skill"
        end
    end
end

function frontmatter_has_key
    set output_path $argv[1]
    set key $argv[2]

    awk -v key="$key" '
        NR == 1 && $0 == "---" { in_fm = 1; next }
        in_fm && $0 == "---" { exit }
        in_fm && $0 ~ "^" key ": " { found = 1; exit }
        END { exit found ? 0 : 1 }
    ' "$output_path"
end

function frontmatter_has_metadata_key
    set output_path $argv[1]
    set key $argv[2]

    awk -v key="$key" '
        NR == 1 && $0 == "---" { in_fm = 1; next }
        in_fm && $0 == "metadata:" { in_metadata = 1; next }
        in_metadata && $0 ~ "^  " key ": " { found = 1; exit }
        in_metadata && ($0 == "---" || $0 ~ "^[^ ]") { exit }
        END { exit found ? 0 : 1 }
    ' "$output_path"
end

function generate_skill
    set skill $argv[1]
    validate_skill "$skill"

    set variants (yq -r ".skills.\"$skill\".variants | keys | .[]" "$rules_file")

    for variant in $variants
        set source_rel (yq_value ".skills.\"$skill\".variants.\"$variant\".source")
        set output_rel (yq_value ".skills.\"$skill\".variants.\"$variant\".output")
        set source_path "$root_dir/$source_rel"
        set output_path "$root_dir/$output_rel"

        if not test -f "$source_path"
            die 2 "rules.yaml parse error: $skill variant $variant source not found: $source_rel"
        end

        set fingerprint (compute_fingerprint "$skill" "$variant")
        or exit $status

        set work_path (mktemp "/tmp/spectra-plus-$skill-$variant.XXXXXX")
        command cp -f "$source_path" "$work_path"

        set count (yq_value ".skills.\"$skill\".transformations | length")
        for index in (seq 0 (math $count - 1))
            set target (yq_value ".skills.\"$skill\".transformations[$index].target_section")
            set op (yq_value ".skills.\"$skill\".transformations[$index].operation")
            set template_name (yq_value ".skills.\"$skill\".transformations[$index].template // \"\"")
            set next_path (mktemp "/tmp/spectra-plus-$skill-$variant.XXXXXX")

            apply_transform "$skill" "$source_path" "$work_path" "$next_path" "$target" "$op" "$template_name"
            or exit 1

            command mv -f "$next_path" "$work_path"
        end

        set renumbered_path (mktemp "/tmp/spectra-plus-$skill-$variant.XXXXXX")
        renumber_step_headings "$work_path" "$renumbered_path"
        command mv -f "$renumbered_path" "$work_path"

        set next_path (mktemp "/tmp/spectra-plus-$skill-$variant.XXXXXX")
        remove_frontmatter_field "$work_path" "$next_path" compatibility
        command mv -f "$next_path" "$work_path"

        for key in name description
            set value (yq_value ".skills.\"$skill\".metadata.\"$key\"")
            set next_path (mktemp "/tmp/spectra-plus-$skill-$variant.XXXXXX")
            set_frontmatter_field "$work_path" "$next_path" "$key" "$value"
            command mv -f "$next_path" "$work_path"
        end

        set meta_keys (yq -r ".skills.\"$skill\".metadata | keys | .[]" "$rules_file")
        for key in $meta_keys
            contains "$key" name description; and continue
            set value (yq_value ".skills.\"$skill\".metadata.\"$key\"")
            set next_path (mktemp "/tmp/spectra-plus-$skill-$variant.XXXXXX")
            set_frontmatter_metadata_field "$work_path" "$next_path" "$key" "$value"
            command mv -f "$next_path" "$work_path"
        end

        set next_path (mktemp "/tmp/spectra-plus-$skill-$variant.XXXXXX")
        set_frontmatter_metadata_field "$work_path" "$next_path" spectraPlusFingerprint "$fingerprint"
        command mv -f "$next_path" "$work_path"

        set sub_path (mktemp "/tmp/spectra-plus-$skill-$variant.XXXXXX")
        apply_substitutions "$skill" "$variant" "$work_path" "$sub_path"
        command rm -f "$work_path"

        set final_path (mktemp "/tmp/spectra-plus-$skill-$variant.XXXXXX")
        insert_marker_after_frontmatter "$sub_path" "$final_path"
        command rm -f "$sub_path"
        validate_generated "$final_path" "$skill"

        echo "$skill|$variant|$output_path|$final_path"
    end
end

set positional
set fingerprint_mode false
set rest $argv
while test (count $rest) -gt 0
    set arg $rest[1]
    set -e rest[1]

    switch $arg
        case -h --help
            usage
            exit 0
        case --root
            if test (count $rest) -eq 0
                die 2 "--root requires a directory argument"
            end
            set root_dir (realpath "$rest[1]")
            set -e rest[1]
        case --fingerprints
            set fingerprint_mode true
        case '*'
            set positional $positional $arg
    end
end

validate_rules
validate_plus_metadata_consistency
validate_all_skills

if test "$fingerprint_mode" = true
    if test (count $positional) -ne 0
        die 2 "--fingerprints does not accept a skill name"
    end

    set fingerprint_rows
    for skill in (fingerprint_skill_names)
        for variant in claude codex
            if not yq -e ".skills.\"$skill\".variants | has(\"$variant\")" "$rules_file" >/dev/null
                die 2 "rules.yaml parse error: $skill missing fingerprint query variant $variant"
            end
            set fingerprint (compute_fingerprint "$skill" "$variant")
            or exit $status
            set fingerprint_rows $fingerprint_rows (printf '%s\t%s\t%s' "$skill" "$variant" "$fingerprint")
        end
    end
    printf '%s\n' $fingerprint_rows
    exit 0
end

set requested_skills
if test (count $positional) -gt 0
    if test (count $positional) -ne 1
        usage >&2
        exit 2
    end

    set requested_skills $positional[1]
    if not yq -e ".skills | has(\"$requested_skills\")" "$rules_file" >/dev/null
        die 2 "rules.yaml parse error: unknown skill $requested_skills"
    end
else
    set requested_skills (yq -r '.skills | keys | .[]' "$rules_file")
end

set generated
for skill in $requested_skills
    set generated $generated (generate_skill "$skill")
end

for item in $generated
    set parts (string split '|' "$item")
    set skill $parts[1]
    set variant $parts[2]
    set output_path $parts[3]
    set final_path $parts[4]
    mkdir -p (dirname "$output_path")
    or die 4 "failed to create output directory for $output_path"
    command mv -f "$final_path" "$output_path"
    or die 4 "failed to write generated skill: $output_path"
    echo "wrote $output_path"
    echo "validated $skill ($variant)"
end
